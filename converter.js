...
// كاش للأسعار - يتم التحديث كل 30 دقيقة فقط
let ratesCache = {
  data: {},
  timestamp: null,
  lastUpdate: null
...
// جلب جميع الأسعار في طلب واحد كل 30 دقيقة
export async function fetchAllRates() {
  // التحقق من الكاش أولاً - لا نجلب إلا كل 30 دقيقة
  if (shouldUseCache()) {
    console.log('استخدام الكاش - آخر تحديث:', ratesCache.lastUpdate);
  ...
  try {
    ...
    console.log('تم تحديث الأسعار بنجاح:', ratesCache.lastUpdate);
...
// جلب سعر زوج محدد (من الكاش أولاً)
...
export function loadCacheFromStorage() {
  try {
    ...
    if (cached) {
      ratesCache = JSON.parse(cached);
      console.log('تم تحميل الأسعار من الكاش');
    }
  } catch (error) {
